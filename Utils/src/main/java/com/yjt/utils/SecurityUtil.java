...
public class SecurityUtil {
    ...
    public byte[] encryptRSA(String publicKey, String keyData) throws CertificateException, NoSuchPaddingException, BadPaddingException, NoSuchAlgorithmException, IllegalBlockSizeException, UnsupportedEncodingException, InvalidKeyException {
        return encryptRSA(getRsaPublicKey(publicKey), keyData);
    }

    public byte[] encryptRSA(PublicKey publicKey, String keyData) throws NoSuchPaddingException, NoSuchAlgorithmException, InvalidKeyException, UnsupportedEncodingException, BadPaddingException, IllegalBlockSizeException {
        Cipher cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding");
        cipher.init(Cipher.ENCRYPT_MODE, publicKey);
        return cipher.doFinal(keyData.getBytes(Regex.UTF_8.getRegext()));
    }
    ...
    public String encryptAES(String content, String key) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, UnsupportedEncodingException, BadPaddingException, IllegalBlockSizeException {
        KeyGenerator.getInstance("AES").init(128, new SecureRandom(key.getBytes()));
        Cipher cipher = Cipher.getInstance("AES");
        cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(KeyGenerator.getInstance("AES").generateKey().getEncoded(), "AES"));
    ...
    public String decryptAES(String content, String key) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, BadPaddingException, IllegalBlockSizeException, UnsupportedEncodingException {
        KeyGenerator.getInstance("AES").init(128, new SecureRandom(key.getBytes()));
        Cipher cipher = Cipher.getInstance("AES");
        cipher.init(Cipher.DECRYPT_MODE, new SecretKeySpec(KeyGenerator.getInstance("AES").generateKey().getEncoded(), "AES"));
    ...
    public String encryptMD5(byte[] buffer) {
        try {
            return bytesToHexString(MessageDigest.getInstance("MD5").digest(buffer));
    ...
    public String encryptSHA1(byte[] buffer) {
        try {
            return bytesToHexString(MessageDigest.getInstance("SHA-1").digest(buffer));
        } catch (NoSuchAlgorithmException e) {
            e.printStackTrace();
            return null;
        }
